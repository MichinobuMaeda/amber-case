...
const makeTree = (base: string, name: string, src: any) => {
  ...
  if (Array.isArray(src)) {
    return (
      ...
      >
        {level < levelMax
          ? src.map((item, index) => makeTree(key, index.toString(), item))
          : '...'}
      </TreeItem>
    );
  }
...
const Debug = () => {
  ...
  const handleToggle = (event: SyntheticEvent<Element>, nodeIds: string[]) => {
    setExpanded(nodeIds);
  };
