...
describe('SignInPage', () => {
  ...
  + 'and shows the selected sub page.', () => {
    ...
    userEvent.click(screen.getByLabelText(`${i18n.t('email address and password')}`));
    ...
    userEvent.click(screen.getByLabelText(`${i18n.t('sign in with email link')}`));
    expect(screen.getByTestId('signInWithEmailLink-panel')).toBeInTheDocument();

    userEvent.type(screen.getByLabelText(`${i18n.t('E-mail')}`), '@');
    userEvent.type(screen.getByLabelText(`${i18n.t('E-mail')}`), 'b');
    userEvent.type(screen.getByLabelText(`${i18n.t('E-mail')}`), 'c');
    userEvent.type(screen.getByLabelText(`${i18n.t('E-mail')}`), '.');
    userEvent.type(screen.getByLabelText(`${i18n.t('E-mail')}`), 'd');
    userEvent.type(screen.getByLabelText(`${i18n.t('E-mail')}`), 'e');
    expect(screen.queryByText(`${i18n.t('correct your email address')}`)).toBeNull();
  });
});
