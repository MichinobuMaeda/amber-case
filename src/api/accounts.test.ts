import {
  onSnapshot, doc, getDoc, collection, updateDoc, Unsubscribe,
...
describe('listenAccounts(context, uid)', () => {
  ...
  + 'if unsub is not empty.', async () => {
    ...
    mockContext.unsub.set('accounts', () => {});
  ...
  + 'if unsub is empty.', async () => {
    ...
    expect(ret).toBeTruthy();
    expect(mockContext.unsub.get('accounts')!()).toEqual('unsub function for accounts');
    expect(mockContext.unsub.get('groups')!()).toEqual('unsub function for groups');
  });
  ...
  + 'if unsub is not empty.', () => {
    ...
    expect(mockContext.unsub.get('doc path')!()).toEqual('unsub function 0');
  });
});
